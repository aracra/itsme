...
window.isGameRunning = false; // [🔥 Exit Guard]
...
window.refillTickets = function() {
    ...
    // Remove No Ticket Msg if exists
...
window.purchaseItem = async function(cost, type, val, name) {
    ...
    if (window.myInfo.tokens < cost) {
        window.openSheet('❌', '토큰 부족', `보유: ${window.myInfo.tokens}💎 / 필요: ${cost}💎`, '충전이 필요해요.');
        return;
    }
...
window.toggleEffect = async function(id) {
    ...
    const newInv = window.myInfo.inventory.map(i => {
        if (i.type === 'effect') {
            ...
            if (newState) return { ...i, isActive: false }; // Disable others
        }
        return i;
    });
...
window.realStartGame = function() {
    ...
    window.isGameRunning = true; // Flag On
...
function showWinner(w, isFinal) {
    ...
    if (window.myInfo.tickets <= 0) {
        ...
        nb.onclick = () => { if(window.disableVoteScreen) window.disableVoteScreen(); };
    } else {
